...
export default function ClientForms({
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Validate the required half here so every bad field is reported at once,
    // rather than one per round-trip. The server re-validates regardless.
  ...
  return (
    ...
    >
      {/* Tabs */}
      <div role="tablist" aria-label="Forms" className="flex flex-wrap gap-2">
        {([
          ...
          >
            ...
            {"optional" in item && item.optional && (
              <span
                className={`ml-2 font-normal normal-case ${tab === item.id ? "text-background/60" : "text-muted"}`}
              >
                optional
              </span>
            )}
      ...
      <div hidden={tab !== "domain"} className="flex flex-col gap-6">
      ...
      {/* --- SMS Lenz (optional) --- */}
      <div hidden={tab !== "sms"} className="flex flex-col gap-6">
