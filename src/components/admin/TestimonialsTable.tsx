...
import { deleteTestimonial, moveTestimonial } from "@/lib/actions/testimonials";
import type { DbTestimonial } from "@/lib/testimonials";
...
export default function TestimonialsTable({
  ...
  const [toDelete, setToDelete] = useState<DbTestimonial | null>(null);
  const [error, setError] = useState<string | null>(null);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return initialTestimonials;
    return initialTestimonials.filter(
      (t) =>
        t.name.toLowerCase().includes(q) ||
        (t.role ?? "").toLowerCase().includes(q) ||
        t.quote.toLowerCase().includes(q),
    );
  }, [initialTestimonials, query]);
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <p className="rounded-xl border border-border bg-surface/40 px-4 py-12 text-center text-sm text-muted">
          No testimonials{query ? " match your search" : " yet"}.
        </p>
      ) : (
              ...
              {filtered.map((t, i) => (
                <tr key={t.id} className="hover:bg-surface/40">
                  <td className="px-4 py-3 align-top">
                    <p className="font-semibold whitespace-nowrap">{t.name}</p>
                    <p className="mt-0.5 text-xs text-muted">{t.role ?? "—"}</p>
                  </td>
                  <td className="max-w-md px-4 py-3 align-top text-muted">
          ...
          >
            ...
            >
              ...
              <p className="mt-2 text-sm text-muted">
                {toDelete.name}&rsquo;s testimonial will be removed from the
                homepage. This can&rsquo;t be undone.
              </p>
